    ...
    <section data-section-wrapper key={`${section}-${name}`}>
      ...
      <section key={sectionKey}>
        {sortedEntriesWithValues.map((x) =>
          createEntry(x, disabled, sectionKey, getAnswerKey)
        )}
      </section>
    </section>
  );
...
function raiseUpdateEvent(e) {
  const value = e.target.value;
  entry.value = value;

  const event = new CustomEvent("update-selection", {
    detail: { id: e.name, value },
  });
}

function makeOptions(id, value, disabled, descriptor, checked) {
  return (
...
function createEntry(entry, disabled, sectionName, getAnswerKey) {
      ...
      <div data-entry-value>
        {Reflect.ownKeys(options)
          ...
          .map((k) => {
            ...
            return makeOptions(id, k, disabled, option, k == value);
          })}
      </div>
    </div>
  );
